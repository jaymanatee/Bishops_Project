...
class MyDeepSeek:
    ...
    def generate_alerts(self, num_alerts):
        """
        ...
        """
        for tweet_tensor, caption_tensor, ner_tags, sa_label in self.data[:num_alerts]:
            tweet_text = self.destokenize(tweet_tensor)
            caption_text = self.destokenize(caption_tensor)
            ...
            last_alert = self.get_alert(result)
            if last_alert:
