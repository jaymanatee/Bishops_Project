...
def train_step(
    ...
    for _, tweet, caption, ner, sa in tqdm(train_data):
        ...
        ner_accuracy, _, _, _, sa_accuracy = accuracy_fn(predicted_ner, predicted_ner_decoded, ner, predicted_sa, sa)
...
def t_step(model, test_data, accuracy_fn, device):
    ...
    """
    model.eval()
    total_ner_accuracy_raw = 0
    total_ner_accuracy_wo_raw = 0
    total_ner_accuracy_decoded = 0
    total_ner_accuracy_wo_decoded = 0
    total_sa_accuracy = 0
    total_batches = 0
    ...
    for _, tweet, caption, ner, sa in test_data:
        ...
        predicted_ner, predicted_ner_decoded, predicted_sa = model(inputs)
        ner_accuracy_raw, ner_accuracy_wo_raw, ner_accuracy_decoded, ner_accuracy_wo_decoded, sa_accuracy = accuracy_fn(
            predicted_ner, 
            predicted_ner_decoded, 
            ner, 
            predicted_sa, 
            sa)
        total_ner_accuracy_raw += ner_accuracy_raw.item()
        total_ner_accuracy_wo_raw += ner_accuracy_wo_raw.item()
        total_ner_accuracy_decoded += ner_accuracy_decoded.item()
        total_ner_accuracy_wo_decoded += ner_accuracy_wo_decoded.item()
        total_sa_accuracy += sa_accuracy.item()
        total_batches += 1

        ner_pred_list = torch.argmax(predicted_ner, dim=-1).tolist()
    ...
    ner_accuracy_raw = total_ner_accuracy_raw / total_batches
    ner_accuracy_wo_raw = total_ner_accuracy_wo_raw / total_batches
    ner_accuracy_decoded = total_ner_accuracy_decoded / total_batches
    ner_accuracy_wo_decoded = total_ner_accuracy_wo_decoded / total_batches
    sa_accuracy = total_sa_accuracy / total_batches

    return ner_accuracy_raw, ner_accuracy_wo_raw, ner_accuracy_decoded, ner_accuracy_wo_decoded, sa_accuracy, all_data
